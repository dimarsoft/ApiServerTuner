...
from predict.yolo_predict import predict_request_yolo
...
@predict_pages.post('/image_predict_yolo')
async def image_predict_yolo(image: UploadFile = File(...)):
    start_time = time_synch()

    print(f"start_time = {start_time}")

    answer = await predict_request_yolo(image)
